---
title: Features
description: Features enabled by default in the Akar preset.
package: unocss-preset
---

When enabling the Akar preset, several features are activated by default to enhance your development experience. These features include:

## Layers

Using [CSS layers](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/At-rules/@layer) to organize styles and manage specificity, this preset adds the `akar` layer with the value of 10. This ensures that Akar styles have higher specificity compared to other layers.

Combining with UnoCSS's built-in [layers](https://unocss.dev/config/layers) mechanism we can utilize these type of classes `uno-layer-akar:mt-10`. But writing that `uno-layer` prefix is tedious, so within this preset we provide a shorthand syntax `akar:mt-10` to apply styles within the `akar` layer.

## Animations

By enabling `akar` preset, you gain access to a variety of pre-defined animation utilities that will be suitable while using the [Akar library](https://akar.vinicunca.dev). The default animations included are:
